# -*- coding: utf-8 -*-
"""DamageModel_Forward_240723.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1TM0GBgj53V8ertuqzJPukga75jUjHTAZ
"""
...
Lx = 0.6 # x-length of the steel plate[m]
c = 4773.343  # [m/s]
...
lb = np.array([0.0, 0.0, 0.0]) # (x_0,y_0,t_0)
ub =  np.array([0.6,0.6,tsim])  # (x_n,y_n,t_n)

# Load Data
data = scipy.io.loadmat('wave2d_damage_06m.mat')
...
def trainingdata(N_u_ic, N_u_bc, N_f ,N_d):
  ...
  X_b3 = np.concatenate([x_b3,y_b3,t_b3],1)  # concatenate x_b3, y_b3, t_b3, x
  print('X_b3 : ', np.shape(X_b3))
  u_b3 = u_star[y_star == y_star.min()][:,None]
  print('t_b3 : ', np.shape(t_b3),'x_b3 : ', np.shape(x_b3),
        'y_b3 : ', np.shape(y_b3),'u_b3 : ', np.shape(u_b3))
  ...
  X_b4 = np.concatenate([x_b4,y_b4,t_b4],1)  # concatenate x_b4, y_b4,t_b4
  print('X_b4 : ', np.shape(X_b4))
  u_b4 = u_star[y_star == y_star.max()][:,None]
  print('t_b4 : ', np.shape(t_b4),'x_b4 : ', np.shape(x_b4),
        'y_b4 : ', np.shape(y_b4),'u_b4 : ', np.shape(u_b4))

  #---------------------------------IC-------------------------------------------
  all_X_u_ic_train = np.vstack([X_b0])  # (,3) = (x,y,t)
  print('all_X_u_ic_train : ', np.shape(all_X_u_ic_train))
  all_u_ic_train = np.vstack([u_b0]) # (,1) = u0
  ...
  all_X_u_bc_train = np.vstack([X_b1, X_b2, X_b3, X_b4])  # (,3) = (x,y,t)
  ...
  return X_f_train, X_u_ic_train, u_ic_train, X_u_bc_train, u_bc_train

"""**4. PINN class**"""

class Forwardmodel(tf.Module):
  ...
  def evaluate(self,x):  # x - inputs of neurons
    x = (x-lb)/(ub-lb) # if x = [100,3] then a = [100,3], #feature scaling
  ...
  def loss_IC(self,x,y): # x = [100,2]   x - network output, y = true values
    loss_u_ic = tf.reduce_mean(tf.square(y-self.evaluate(x)))  # y_cap[100,1] - y_true[100,1]

    # implementing du_dt IC
    g = tf.Variable(x, dtype='float64', trainable=False) # (6000, 3)
    x_f = g[:,0:1] # (6000, 1)
    y_f = g[:,1:2] # (6000, 1)
    t_f = g[:,2:3] # (6000, 1)
    ...
    with tf.GradientTape(persistent=True) as tape:
      tape.watch(t_f)
      g = tf.stack([x_f[:,0], y_f[:,0], t_f[:,0]],axis=1) # (6000, 3)
  ...
  def loss_BC(self,x,y): # x = [100,2]   x - network output, y = true values
    loss_bc = tf.reduce_mean(tf.square(y-self.evaluate(x)))  # y_cap[100,1] - y_true[100,1]
    return loss_bc

  def loss_PDE(self,x_to_train_f): # x_to_train_f = g = (7000, 3) - (x,y,t)
    g = tf.Variable(x_to_train_f, dtype='float64', trainable=False) # (7000, 3)
    ...
    with tf.GradientTape(persistent=True) as tape: #To compute multiple gradients over the same computation, create a gradient tape with persistent=True
      ...
      with tf.GradientTape(persistent=True) as doubletape:
        ...
        g = tf.stack([x_f[:,0], y_f[:,0], t_f[:,0]],axis=1) # (7000, 3)
        #print('g: ', np.shape(g))
        ...
        # Damage calculation
        x_part = ((x_f - x_0)**2)/(2*sigma_x**2)
        y_part = ((y_f - y_0)**2)/(2*sigma_y**2)
        damage = A*tf.exp(-(x_part + y_part)) # (7000,1)
  ...
  def predict(self, x_test,y_test,t_test):
      X_u_test = np.stack([x_test[:,0], y_test[:,0], t_test[:,0]],axis=1)
...
N_u_ic = 100
N_u_bc =  100 # Total number of data points for 'u'
...
'''
X_f_train, X_u_ic_train, u_ic_train, X_u_bc_train, u_bc_train = trainingdata(N_u_ic, N_u_bc, N_f ,N_d)

# network architecture
layers = np.array([3,20,20,20,20,20,20,20,20, 1])

PINN = Forwardmodel(layers)
...
#PDE_loss = PINN.loss_PDE(X_f_train)
...
# Prediction
u_pred = PINN.predict(x_test,y_test,t_test)
...
plt.savefig('wave2d_8n.png',dpi = 600)